...
def parse_hs0101_data(hex_string):
    ...
    try:
        if len(hex_string) != 32:
            print(f"HS0101: Неправильная длина данных: {len(hex_string)}, ожидается 32")
            return None
        ...
        packet = result[0].lstrip('0')
        battery = int(result[1], 16)
        ...
        door_open = int(result[9], 16) == 0
        door_open2 = int(result[10], 16) == 0
...
def parse_tl11_data(hex_string):
    ...
    try:
        if len(hex_string) != 20:
            print(f"TL11: Неправильная длина данных: {len(hex_string)}, ожидается 20")
            return None
        ...
        temperature2 = int(temp2_hex, 16) / 10
        
        up_temperature = int(result[9], 16)
...
async def fetch_iot_data_async(devEui):
    """Асинхронная функция для получения данных с IoT сервера для конкретного devEui"""
    global iot_server_available
    
    try:
        print(f"Попытка подключения к серверу {SERVER_URL} для DEV_EUI={devEui}...")
...
def fetch_all_iot_data_sync(dev_eui_list):
    """Запускает асинхронные задачи для всех devEui параллельно"""
    async def run_all():
        tasks = [fetch_iot_data_async(dev_eui) for dev_eui in dev_eui_list]
    ...
    try:
        ...
        loop.close()
        return True, "Все данные успешно получены"
    except Exception as e:
        return False, f"Ошибка: {str(e)}"

def fetch_iot_data_sync():
    """Синхронная обертка для обратной совместимости"""
    return fetch_all_iot_data_sync(DEV_EUI_LIST)

# Фоновая задача для получения данных (раз в 5 минут)
def background_fetch():
    """Фоновая задача для регулярного получения данных каждые 5 минут"""
    ...
    while True:
        try:
            ...
            if current_time - last_fetch_time >= DATA_FETCH_INTERVAL:
                print(f"\n{'='*50}")
                print(f"🔄 Начало получения данных ({datetime.now().strftime('%H:%M:%S')})")
                print(f"Последнее получение: {time.ctime(last_fetch_time) if last_fetch_time > 0 else 'никогда'}")
                ...
                if remaining > 60:  # Показываем только если больше минуты
...
def index():
    ...
    # Вычисляем время до следующего получения
    global last_fetch_time
    current_time = time.time()
    if last_fetch_time > 0:
        time_since_last = int(current_time - last_fetch_time)
        time_until_next = max(0, DATA_FETCH_INTERVAL - time_since_last)
        minutes = time_until_next // 60
        seconds = time_until_next % 60
        next_fetch_info = f"{minutes} мин {seconds} сек"
        last_fetch_str = time.ctime(last_fetch_time)
    else:
        next_fetch_info = "ожидание первого получения"
        last_fetch_str = "никогда"
...
def receive_sensor_data():
    ...
    try:
        ...
        if len(hex_data) == 32:  # HS0101
            ...
            server_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            cur.execute('''
                INSERT INTO measurements (device_id, received_at, raw_data)
                VALUES (?, ?, ?)
            ''', (device_id, server_time, hex_data))
            conn.commit()
            conn.close()
